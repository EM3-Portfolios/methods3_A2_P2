...
#old name: "model8"
chi_mlu2 <- lmer(CHI_MLU ~ Age * ADOS2 + ExpressiveLangRaw.x + (1 | Child.ID), new_df)

library(MuMIn)

r.squaredGLMM(chi_mlu1)
r.squaredGLMM(chi_mlu2)

chi_mlu1 <- lmer(CHI_MLU ~ VISIT + VISIT2 + Diagnosis + VISIT*Diagnosis + (1 + Diagnosis + MOT_MLU | SUBJ), df1)
#old name: "pm2_quadratic"
mot_mlu <- lmer(MOT_MLU ~ VISIT + Diagnosis + VISIT2 + VISIT*Diagnosis + (1 + CHI_MLU | SUBJ), df1)
#old name: "model8"
chi_mlu2 <- lmer(CHI_MLU ~ I(Age^2) * I(ADOS2^2) + I(ExpressiveLangRaw^2) + (1 | SUBJ), df1)

#- calculate performance of the model on the training data: root mean square error is a good measure. (Tip: google the function rmse())


predictions <- predict(chi_mlu2, df)

rmse(chi_mlu1$df, predictions)

predictions <- fitted(chi_mlu1, df)
