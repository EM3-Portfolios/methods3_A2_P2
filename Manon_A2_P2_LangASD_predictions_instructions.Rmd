...
```{r optional}
#- optional: predictions are never certain, can you identify the uncertainty of the predictions? (e.g. google predictinterval())
# optional1 <- predictInterval(model8, CleanTest_df, allow.new.levels = TRUE)
...
# before we do this we need to remove the rows where some specific variables (CHI_MLU, others) are not available, in order for the cross-validation loop to work.
...
# model8 is more generalizable than the basic model if the difference between train and test is smaller for model 8 than the basic.?
```
