...
pacman::p_load(readr,dplyr,stringr,lmerTest,Metrics,caret, lme4, caret)
...
for (fold in folds){
  ...
  model = lmer(CHI_MLU ~ Age * I(ADOS2^2) + ExpressiveLangRaw + (1 | Child.ID), df)
  test$prediction <- predict(model, test, allow.new.levels = TRUE)
  train$prediction <- fitted(model) #this is the line that causes the error "replacement has x rows, data has y" - We think.
...
# I will just try to only use RMSE() instead of this loop that we can't figure out.
# OK, so using the example from the defaultSummary {caret} help page:
predicted_A2 <- fitted(model8)
observed_A2 <- df$CHI_MLU
observed_A2 <- na.omit(observed_A2)
RMSE(predicted_A2, observed_A2)

length(predicted_A2)
length(observed_A2)
#they are still not the same length....
