...
new_df <- merge(new_df, subset, by = "Child.ID")
  
new_df$Visit2 <- df$Visit^2
...
#old name: "quadratic"
chi_mlu1 <- lmer(CHI_MLU ~ Visit + Visit2 + Diagnosis + Visit*Diagnosis + (1 + Diagnosis + MOT_MLU | Child.ID), new_df)

#old name: "model8"
chi_mlu2 <- lmer(CHI_MLU ~ Age * I(ADOS2^2) + ExpressiveLangRaw + (1 | Child.ID), new_df)
...
for (fold in folds){
  train = subset(new_df, !(Child.ID %in% fold))
  test = subset(new_df, Child.ID %in% fold)
  model = chi_mlu1 # or lmer(CHI_MLU ~ Visit + Visit2 + Diagnosis + Visit*Diagnosis + (1 + Diagnosis + MOT_MLU | Child.ID), train)
