...
new_df <- read_csv("data_clean.csv")
new_df$X1 <- NULL

# Subsetting the columns that we want to work with:
subset <- select(new_df, Child.ID, Visit, ADOS, MullenRaw, ExpressiveLangRaw, Socialization)
...
df <- merge(new_df, subset, by = "Child.ID")
...
#old name: "quadratic"
chi_mlu1 <- lmer(CHI_MLU ~ Visit + I(Visit^2) + Diagnosis + Visit*Diagnosis + (1 + Diagnosis + MOT_MLU | Child.ID), df)
...
#- calculate performance of the model on the training data: root mean square error is a good measure. (Tip: google the function rmse())
sub <- df%>%
  dplyr::select(c(CHI_MLU, Visit, Diagnosis, Child.ID, ADOS2, ExpressiveLangRaw, Age))%>%
  na.omit()

folds <- createFolds(unique(sub$Child.ID), k = 6, list = TRUE, returnTrain = FALSE)
trainRMSE <- rep(NA, k)
testRMSE <- rep(NA, k)
i <- 1

for (fold in folds){
  train = subset(sub, !(Child.ID %in% fold))
  test = subset(sub, Child.ID %in% fold)
  model = lmer(CHI_MLU ~ Age * I(ADOS2^2) + ExpressiveLangRaw + (1 | Child.ID), train)
