...
for (fold in folds){
  train = subset(df, !(Child.ID %in% fold))
  test = subset(df, Child.ID %in% fold)
  model = lmer(CHI_MLU ~ Age * I(ADOS1^2) + verbalIQ1 + (1 + Age | Child.ID), train)
  test$prediction <- predict(model, test, allow.new.levels = TRUE)
  train$prediction <- fitted(model)
  trainRMSE_basic[i] <- RMSE(train$CHI_MLU, fitted(model))
  testRMSE_basic[i] <- RMSE(test$CHI_MLU, test$prediction)
...
for (fold in folds){
  train = subset(df, !(Child.ID %in% fold))
  test = subset(df, Child.ID %in% fold)
  model = lmer(CHI_MLU ~ Age * I(ADOS1^2) + nonVerbalIQ1 + (1 | Child.ID), train)
  test$prediction <- predict(model, test, allow.new.levels = TRUE)
  train$prediction <- fitted(model)
  trainRMSE_basic[i] <- RMSE(train$CHI_MLU, fitted(model))
  testRMSE_basic[i] <- RMSE(test$CHI_MLU, test$prediction)
...
crossValidation2 <- data.frame(trainRMSE_basic, testRMSE_basic, trainRMSE_m8, testRMSE_m8, trainRMSE_m1, testRMSE_m1, trainRMSE_m2, testRMSE_m2, trainRMSE_m3, testRMSE_m3, trainRMSE_m4, testRMSE_m4)
