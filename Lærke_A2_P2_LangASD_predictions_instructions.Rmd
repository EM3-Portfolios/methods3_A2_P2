...
model8 <- lmer(CHI_MLU ~ Age * I(ADOS2^2) + ExpressiveLangRaw + (1 | Child.ID), df)
...
CleanTest_df <- rename(CleanTest_df, ADOS2 = Ados1)



# NB: this dataset (CleanTest_df) is really, really small
```
...
for (fold in folds){
  ...
  trainRMSE_basic[i] <- RMSE(train$types_CHI, fitted(model))
  testRMSE_basic[i] <- RMSE(test$types_CHI, test$prediction)
  i <- i + 1
}
...
for (fold in folds){
  train = subset(df, !(Child.ID %in% fold))
  test = subset(df, Child.ID %in% fold)
  model = model8 <- lmer(CHI_MLU ~ Age * I(ADOS2^2) + ExpressiveLangRaw2 + (1 | Child.ID), train)
  test$prediction <- predict(model, test, allow.new.levels = TRUE)
  train$prediction <- fitted(model)
