...
model8 <- lmer(CHI_MLU ~ Age * I(ADOS1^2) + verbalIQ1 + (1 | Child.ID), df)

# Comparing the R^2 values for the two models:
r.squaredGLMM(quadratic)
r.squaredGLMM(model8)

# Comparing the AIC and BIC values for the two models:
m_aic <- AIC(quadratic, model8)
m_bic <- BIC(quadratic, model8)
...
# Model 8 has higher R^2 and lower AIC and BIC values.
```
...
```{r}
#- test the performance of the models on the test data (Tips: google the functions "predict()")
predictions_againstTrain <- predict(model8, CleanTest_df, allow.new.levels = TRUE)
...
crossValidation <- data.frame(trainRMSE_basic, testRMSE_basic, trainRMSE_m8, testRMSE_m8)
crossValidation$basic_diff <- abs(crossValidation$trainRMSE_basic - crossValidation$testRMSE_basic)
crossValidation$m8_diff <- abs(crossValidation$trainRMSE_m8 - crossValidation$testRMSE_m8)

mean(crossValidation$basic_diff)
mean(crossValidation$m8_diff)

mean(crossValidation$trainRMSE_basic)
mean(crossValidation$testRMSE_basic)
mean(crossValidation$trainRMSE_m8)
mean(crossValidation$testRMSE_m8)
```
...
basic <- lmer(CHI_MLU ~ Visit*Diagnosis + (1 + Diagnosis + MOT_MLU | Child.ID), df) # Used to be called lm4
...
for (fold in folds){
  train = subset(df, !(Child.ID %in% fold))
  test = subset(df, Child.ID %in% fold)
  model = lmer(CHI_MLU ~ I(Visit^2) * Diagnosis + verbalIQ1 + (1 + Visit | Child.ID), train)
  test$prediction <- predict(model, test, allow.new.levels = TRUE)
  train$prediction <- fitted(model)
...
for (fold in folds){
  train = subset(df, !(Child.ID %in% fold))
  test = subset(df, Child.ID %in% fold)
  model = lmer(CHI_MLU ~ Age * I(ADOS1^2) + verbalIQ1 + Socialization1 + (1 + Age | Child.ID), train)
  test$prediction <- predict(model, test, allow.new.levels = TRUE)
  train$prediction <- fitted(model)
...
for (fold in folds){
  train = subset(df, !(Child.ID %in% fold))
  test = subset(df, Child.ID %in% fold)
  model = lmer(CHI_MLU ~ Age * I(ADOS1^2) + verbalIQ1 + Socialization1 + (1 + Age + MOT_MLU | Child.ID), train)
  test$prediction <- predict(model, test, allow.new.levels = TRUE)
  train$prediction <- fitted(model)
...
for (fold in folds){
  train = subset(df, !(Child.ID %in% fold))
  test = subset(df, Child.ID %in% fold)
  model = lmer(CHI_MLU ~ Visit + I(Visit^2) + Diagnosis + Visit*Diagnosis + (1 + Diagnosis + MOT_MLU | Child.ID), train)
  test$prediction <- predict(model, test, allow.new.levels = TRUE)
  train$prediction <- fitted(model)
...
for (fold in folds){
  train = subset(df, !(Child.ID %in% fold))
  test = subset(df, Child.ID %in% fold)
  model = lmer(CHI_MLU ~ Visit + I(Visit^2) + Diagnosis + Visit*Diagnosis + (1 | Child.ID), train)
  test$prediction <- predict(model, test, allow.new.levels = TRUE)
  train$prediction <- fitted(model)
...
for (fold in folds){
  train = subset(df, !(Child.ID %in% fold))
  test = subset(df, Child.ID %in% fold)
  model = lmer(CHI_MLU ~ Age * I(ADOS1^2) + (1 | Child.ID), train)
  test$prediction <- predict(model, test, allow.new.levels = TRUE)
  train$prediction <- fitted(model)
...
crossValidation2 <- data.frame(trainRMSE_basic, testRMSE_basic, trainRMSE_m8, testRMSE_m8, trainRMSE_m1, testRMSE_m1, trainRMSE_m2, testRMSE_m2, trainRMSE_m3, testRMSE_m3, trainRMSE_m4, testRMSE_m4, trainRMSE_m5, testRMSE_m5, trainRMSE_m6, testRMSE_m6)

mean(crossValidation2$trainRMSE_basic)
mean(crossValidation2$testRMSE_basic)
mean(crossValidation2$trainRMSE_m8)
mean(crossValidation2$testRMSE_m8)
mean(crossValidation2$trainRMSE_m1)
mean(crossValidation2$testRMSE_m1)
mean(crossValidation2$trainRMSE_m2)
mean(crossValidation2$testRMSE_m2)
mean(crossValidation2$trainRMSE_m3)
mean(crossValidation2$testRMSE_m3)
mean(crossValidation2$trainRMSE_m4)
mean(crossValidation2$testRMSE_m4)
mean(crossValidation2$trainRMSE_m5)
mean(crossValidation2$testRMSE_m5)
mean(crossValidation2$trainRMSE_m6)
mean(crossValidation2$testRMSE_m6)

mean_train <- c(mean(crossValidation2$trainRMSE_basic), mean(crossValidation2$trainRMSE_m8), mean(crossValidation2$trainRMSE_m1), mean(crossValidation2$trainRMSE_m2), mean(crossValidation2$trainRMSE_m3), mean(crossValidation2$trainRMSE_m4), mean(crossValidation2$trainRMSE_m5), mean(crossValidation2$trainRMSE_m6))

mean_test <- c(mean(crossValidation2$testRMSE_basic), mean(crossValidation2$testRMSE_m8), mean(crossValidation2$testRMSE_m1), mean(crossValidation2$testRMSE_m2), mean(crossValidation2$testRMSE_m3), mean(crossValidation2$testRMSE_m4), mean(crossValidation2$testRMSE_m5), mean(crossValidation2$testRMSE_m6))

models <- c("basic", "m8", "m1", "m2", "m3", "m4", "m5", "m6")

crossValidation2Mean <- data.frame(models, mean_train, mean_test)
...
```{r}
m1 <- lmer(CHI_MLU ~ I(Visit^2) * Diagnosis + verbalIQ1 + (1 + Visit | Child.ID), train)
...
# The average residual (difference between observed and predicted value) is 0.48 from the observed traning data to the model predictions.
