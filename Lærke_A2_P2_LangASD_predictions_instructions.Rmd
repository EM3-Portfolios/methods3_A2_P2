...
# Adding our squared visit column
...
#old name: "quadratic"
chi_mlu1 <- lmer(CHI_MLU ~ Visit + Visit2 + Diagnosis + Visit*Diagnosis + (1 + Diagnosis + MOT_MLU | Child.ID), df)
#old name: "pm2_quadratic"
  # mot_mlu <- lmer(MOT_MLU ~ VISIT + Diagnosis + VISIT2 + VISIT*Diagnosis + (1 + CHI_MLU | SUBJ), df)
#old name: "model8"
chi_mlu2 <- lmer(CHI_MLU ~ Age * ADOS1 + ExpressiveLangRaw + (1 | Child.ID), df)
```
...
DF <- completeFun(DF, "ADOS2")

DF <- completeFun(DF, "ExpressiveLangRaw")
```


```{r, include = FALSE}
pacman::p_load("caret", "lme4")
...
for (fold in folds){
  ...
  model = lmer(CHI_MLU ~ VISIT + Diagnosis + VISIT*Diagnosis + (1 | SUBJ), DF)
...
for (fold in folds){
  train = subset(DF, !(SUBJ %in% fold))
  test = subset(DF, SUBJ %in% fold)
  model = lmer(CHI_MLU ~ VISIT + Visit2 + Diagnosis + VISIT*Diagnosis + (1 + Diagnosis + MOT_MLU | SUBJ), DF)
  test$prediction <- predict(model, test, allow.new.levels = TRUE)
  train$prediction <- fitted(model)
  trainRMSE[i] <- RMSE(train$types_CHI, fitted(model))
  testRMSE[i] <- RMSE(test$types_CHI, test$prediction)
...
for (fold in folds){
  ...
  model = lmer(CHI_MLU ~ Age * ADOS1 + ExpressiveLangRaw + (1 | Child.ID), df)
  test$prediction <- predict(model, test, allow.new.levels = TRUE)
  train$prediction <- fitted(model)
  trainRMSE[i] <- RMSE(train$types_CHI, fitted(model))
  testRMSE[i] <- RMSE(test$types_CHI, test$prediction)
